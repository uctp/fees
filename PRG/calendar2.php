<div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main" id="content">
    ...
    <link rel="stylesheet" type="text/css" href="<?php echo base_url('assets/css/frontierCalendar/jquery-frontier-cal-1.3.2.css'); ?>" />

    <!-- Include CSS for color picker plugin (Not required for calendar plugin. Used for example.) -->
    <link rel="stylesheet" type="text/css" href="<?php echo base_url('assets/css/colorpicker/colorpicker.css'); ?>" />

    <!-- Include CSS for JQuery UI (Required for calendar plugin.) -->
    <link rel="stylesheet" type="text/css" href="<?php echo base_url('assets/css/jquery-ui/smoothness/jquery-ui-1.8.1.custom.css'); ?>" />
    ...
    ** This is our IE fix version which enables drag-and-drop to work correctly in IE. See README file in js/jquery-core folder. **
    -->
    <script type="text/javascript" src="<?php echo base_url('assets/js/jquery-core/jquery-1.4.2-ie-fix.min.js'); ?>"></script>

    <!-- Include JQuery UI (Required for calendar plugin.) -->
    <script type="text/javascript" src="<?php echo base_url('assets/js/jquery-ui/smoothness/jquery-ui-1.8.1.custom.min.js'); ?>"></script>

    <!-- Include color picker plugin (Not required for calendar plugin. Used for example.) -->
    <script type="text/javascript" src="<?php echo base_url('assets/js/colorpicker/colorpicker.js'); ?>"></script>

    <!-- Include jquery tooltip plugin (Not required for calendar plugin. Used for example.) -->
    <script type="text/javascript" src="<?php echo base_url('assets/js/jquery-qtip-1.0.0-rc3140944/jquery.qtip-1.0.js'); ?>"></script>
    ...
    -->
    <script type="text/javascript" src="<?php echo base_url('assets/js/lib/jshashtable-2.1.js'); ?>"></script>

    <!-- Include JQuery Frontier Calendar plugin -->
    <script type="text/javascript" src="<?php echo base_url('assets/js/frontierCalendar/jquery-frontier-cal-1.3.2.min.js'); ?>"></script>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            $("#content").children().css("font-size", "62.5%");
            var clickDate = "";
            var clickAgendaItem = "";
...
if ($events) {
    foreach ($events as $event) {
        ...
        $created_date_time = explode(" ", $event['created_date']);
        $created_date = explode("-", $created_date_time[0]);
        $created_time = explode(":", $created_date_time[1]);
        $from_time = explode(":", $event['from_time']);
        $to_time = explode(":", $event['to_time']);
        ?>
                    var startDateObj = new Date(parseInt(<?php echo $from_date[0]; ?>), parseInt(<?php echo $from_date[1]; ?>) - 1, parseInt(<?php echo $from_date[2]; ?>), <?php echo $from_time[0]; ?>, <?php echo $from_time[1]; ?>, 0, 0);
                    var endDateObj = new Date(parseInt(<?php echo $to_date[0]; ?>), parseInt(<?php echo $to_date[1]; ?>) - 1, parseInt(<?php echo $to_date[2]; ?>), <?php echo $to_time[0]; ?>, <?php echo $to_time[1]; ?>, 0, 0);
                    var createdDateObj = new Date(parseInt(<?php echo $created_date[0]; ?>), parseInt(<?php echo $created_date[1]; ?>) - 1, parseInt(<?php echo $created_date[2]; ?>), <?php echo $created_time[0]; ?>, <?php echo $created_time[1]; ?>, 0, 0);
                    jfcalplugin.addAgendaItem(
                            "#mycal",
                            "<?php echo $event['schedule_name']; ?>",
                            startDateObj,
                            endDateObj,
                            false,
                            {
                                createdDate: createdDateObj,
                                schedule_id: <?php echo $event['schedule_id']; ?>
                            },
                    {
...
<?php } else { ?>
                    $("#display-event-form-read-only").dialog('open');
<?php } ?>
            }
            ;
                ...
                buttons: {
                    'Add Event': function() {
                        ...
                        if (what == "") {
                            ...
                            var faction = "<?php echo site_url('user/add_event'); ?>";
                            var fdata = {};
                            fdata.event_name = what;
                            fdata.start_date = startDate;
                            fdata.end_date = endDate;
                            fdata.start_time = startHour + ":" + startMin;
                            fdata.end_time = endHour + ":" + endMin;
                            fdata.total_strength = $("#total_strength").val();
...
//                            var schedule_id = "";
                    ...
                    'Delete': function() {
                        if (confirm("Are you sure you want to delete this agenda item?")) {
                            if (clickAgendaItem != null) {
                                var faction = "<?php echo site_url('user/delete_event'); ?>";
                                var fdata = {};
                                fdata.schedule_id = clickAgendaItem.data.schedule_id;
                                $.post(faction, fdata, function(rdata) {
            ...
            /**
             * Initialize ready only display event form for students.
             */
            $("#display-event-form-read-only").dialog({
                autoOpen: false,
                height: 400,
                width: 400,
                modal: true,
                ...
                open: function(event, ui) {
                    if (clickAgendaItem != null) {
                        ...
                        var data = clickAgendaItem.data;
                        // in our example add agenda modal form we put some fake data in the agenda data. we can retrieve it here.
                        $("#display-event-form-read-only").append(
                                "<br><b>" + title + "</b><br><br>"
                                );
                        if (allDay) {
                            $("#display-event-form-read-only").append(
                        ...
                        for (var propertyName in data) {
                            $("#display-event-form-read-only").append("<b>" + propertyName + ":</b> " + data[propertyName] + "<br>");
                        }
                    }
                },
                close: function() {
                    // clear agenda data
                    $("#display-event-form-read-only").html("");
                }
            });
                ...
                <table style="width:100%; padding:5px;">
                    ...
                    <tr>
                        <td>
                            ...
                            <input type="text" name="location" id="location" value="" style="margin-bottom:12px; width:95%; padding: .4em;"/>
                        </td>
                        <td>&nbsp;</td>
                        <td>
                            <label>Strength</label>
                            <input type="text" name="total_strength" id="total_strength" value="" style="margin-bottom:12px; width:95%; padding: .4em;"/>
                        </td>
                    </tr>
                </table>
                <table>
    ...
    <div id="display-event-form-read-only" title="View Agenda Item">
